...
export class PathController {
  ...
  async all(request: Request, response: Response, next: NextFunction) {
    ...
    let paths = await this.pathRepository.find({
      where: { ...request.query },
      relations: ["start", "end"]
    });
    try {
      if (!paths.length && type && type !== "Indoor") {
        ...
        let routes;
        if (type === "Walk") {
          searchString = `${start.lng},${start.lat};${end.lng},${
            end.lat
          }?alternatives=true`;
          const {
            data: { routes }
          ...
          for (let route of routes) {
            const { geometry, distance, duration } = route;
            const latLngs = polyline.decode(geometry);
            const walkingPath = await this.jeepRepository.save({
              start,
              end,
              latLngs,
              geometry,
        ...
        paths = await this.pathRepository.find({
          where: { ...request.query },
          relations: ["start", "end"]
        });
      }
        ...
        switch (type) {
          case "jeep":
            if (start instanceof Stop && end instanceof Stop) {
              const path = await this.jeepRepository.save({
                start,
                end,
                latLngs
              });
              return this.jeepRepository.findOne(path.id, {
                relations: ["start", "end"]
              });
            } else {
              return {
          ...
          case "walk":
            if (!(start instanceof Room) && !(end instanceof Room)) {
              const path = await this.walkRepository.save({
                start,
                end,
                latLngs
              });
              return this.walkRepository.findOne(path.id, {
                relations: ["start", "end"]
              });
            } else {
              return {
